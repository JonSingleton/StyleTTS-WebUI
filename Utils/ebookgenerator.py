...
def prepareTexts(txtPath,genData):
	...
	for chapterFile in sorted(os.listdir(txtPath), key=sort_key_texts):
		if chapterFile.endswith('.txt'):
			...
			SCMaxLength = genData['genSettings']['SCMaxLength']
			SCDesiredLength = genData['genSettings']['SCDesiredLength']

			srtIter = 0
			for t in lineList:
				splitList = split_and_recombine_text(t,SCDesiredLength,SCMaxLength)
...
def convert_ebook_to_audio(ebook_file, progress=gr.Progress()):
	...
	genData = {
		...
		'genSettings':{
			'SCDesiredLength': int(currentSettings['SCDesiredLength']),
			'SCMaxLength': int(currentSettings['SCMaxLength'])
		}
	}
	...
	def getETA(start,totalInferences,currentInference):
		...
		secPerInference = ((time.time() - start) / currentInference)
		...
		currentAverageETAReadable = convert_to_readable_time(currentAverageETA)

		print(f"Process Time: {time.time() - start}\n   Inferences: {currentInference}\n   Inferences/second: {currentInference / ((time.time() - start))}\n   Remaining Inferences: {remainingInferences}\n   Time remaining: {inferencesETA}\n   Averaged ETA: {currentAverageETAReadable}")
		
		

		return currentAverageETAReadable
	...
	for i in genData['genPaths']['chapterTexts']:
		...
		for i2 in genData['genPaths']['chapterTexts'][i]['inferences']:
			...
			for t in genData['genPaths']['chapterTexts'][i]['inferences'][i2]:
				currentAverageETA = getETA(start,totalInferences,currentInference)
				progress(currentProgressPercent, desc=f"Inferencing: Chapter {chapterNum} of {totalChapterCount} | Section {i2} of {len(genData['genPaths']['chapterTexts'][i]['inferences'])} | Fragment {fragmentCounter} of {len(genData['genPaths']['chapterTexts'][i]['inferences'][i2])}.\nEstimated ETA: {currentAverageETA}")
